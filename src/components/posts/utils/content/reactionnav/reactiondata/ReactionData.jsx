...
const ReactionData = ({ postId, postReactions, currentAccount }) => {
    ...
    const data = [
        ...
        },
        {
            ...
            onClick: handleLike, // Attach the like handler
        },
        {
            title: 'View',
    ...
    return (
        <div className="relative mr-[49px] flex flex-1 items-center justify-start">
            {data.map((data, index) => (
                ...
                >
                    <button
                        className={`absolute flex min-h-5 cursor-pointer items-center text-[14px] text-[#71767b] ${data.title === 'Repost' ? 'hover:text-[#00ba7c]' : data.title === 'Like' ? 'hover:text-[#f91881]' : 'hover:text-[#1d9bf0]'}`}
                        onClick={(e) => {
                            e.preventDefault();
                            data.onClick?.();
                        }}
