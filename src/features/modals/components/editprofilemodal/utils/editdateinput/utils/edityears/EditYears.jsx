...
import { YearData } from 'features/modals/components/createaccountmodal/formarea/inputs/dateinput/data/yeardata/YearData';
import { DropDownSVG } from 'components/icons/DropDownSVG';

const EditYears = ({ year, setYear }) => {
    const [isFocusedYear, setIsFocusedYear] = useState(false);
    const sortedYeardata = YearData.sort((a, b) => b - a);

    return (
        <div
            className={`${isFocusedYear ? 'border-[#1d9bf0]' : 'border-[#333639]'} relative mt-4 mb-3 flex h-full flex-shrink-0 grow-4 items-center justify-center rounded-[4px] border px-2 py-3 pt-3 pb-2 transition-colors duration-100 ease-in-out`}
        >
