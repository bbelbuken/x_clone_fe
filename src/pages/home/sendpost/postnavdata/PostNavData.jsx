    ...
    return (
        <div className="left-0 flex h-full flex-1 items-center justify-start">
            {navdata.map((item, index) => (
                <button
                    className="flex min-h-9 min-w-9 items-center justify-center rounded-full transition-colors hover:bg-[#1d9bf01a]"
                    key={index}
                    title={item.title}
                    onClick={
                        item.title === 'Media' ? handleFileClick : undefined
                    }
                >
                    <div className="flex min-w-0 grow items-center justify-center text-[15px] leading-5 font-bold break-words text-[#1d9bf0]">
                        {item.icon}
                    </div>
                </button>
            ))}
