...
const Followers = ({ currentAccount, refetch }) => {
    ...
    if (isLoading) {
        ...
        // Filter accounts that are following the current user
        const followerIds = ids.filter((userId) =>
            currentAccount.followers?.includes(userId),
        );

        content = followerIds.map((accountId) => {
        ...
        if (followerIds.length === 0) {
            content = (
                <div className="flex min-h-[200px] items-center justify-center">
                    <p className="text-[15px] text-[#71767b]">No followers.</p>
                </div>
            );
        }
    }
