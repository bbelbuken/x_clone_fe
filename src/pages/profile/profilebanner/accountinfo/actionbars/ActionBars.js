    ...
    <div className="flex max-w-full flex-wrap items-end justify-center">
      {actionBars.map((item, index) => (
        <button
          key={index}
          title={item.title}
          className="mb-3 mr-2 flex min-h-9 min-w-9 items-center justify-center rounded-full border border-[#536471] transition-colors hover:bg-[#eff3f41a]"
        >
          <svg viewBox="0 0 24 24" fill="#eff3f4" height={20} width={20}>
            {item.icon}
          </svg>
        </button>
      ))}
      <Button
        size="profile-follow"
        className={`mb-3 flex min-h-9 min-w-9 items-center justify-center ${isFollowing ? 'min-w-[104px] hover:border-[#f4212f] hover:bg-transparent hover:text-[#f4212f]' : 'min-w-[81px] bg-[#eff3f4] text-black hover:bg-[#d7dbdc]'}`}
