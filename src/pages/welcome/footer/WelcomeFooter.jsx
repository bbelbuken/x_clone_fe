...
const WelcomeFooter = () => {
    ...
    return (
        <div>
            <nav className="flex flex-wrap justify-center px-4 py-6 sm:py-3">
                {WelcomeFooterData.map((item, index) => (
                    <Link
                        key={index}
                        to={item.path}
                        className="my-1 min-w-0 cursor-pointer pr-[14.5px] text-[12px] leading-4 font-normal tracking-[0.025em] break-words text-[#71767b] hover:underline sm:text-[13px]"
                    >
                        {item.title}
                    </Link>
                ))}
